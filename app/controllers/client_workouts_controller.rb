class ClientWorkoutsController < ApplicationController
  ...
  def find
  	...
  	#@client_workouts = ClientWorkout.where("LOWER(client_name) = ?", params[:search_string])
  	@client_workouts = ClientWorkout.where("LOWER(client_name) LIKE ?", "%" + params[:search_string].downcase + "%")
  	@name = params
